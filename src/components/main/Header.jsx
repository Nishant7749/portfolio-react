...
function Header() {
  ...
  <nav>
    <div>
    <ul className='hidden md:flex justify-center gap-4 text-center'>
    ...
    <div>
      <button className="absolute right-6 top-8 flex flex-col gap-1 bg-none border-none cursor-pointer md:hidden " onClick={()=> setIsOpen(!isOpen)}>
        <span className={`bg-white w-8 h-1 rounded-md transition duration-400 ${isOpen ? 'rotate-45 translate-y-2': ''} `}></span>
        <span className={`bg-white w-8 h-1 rounded-md transition duration-400 ${isOpen ? 'opacity-0': ''} `}></span>
        <span className={`bg-white w-8 h-1 rounded-md transition duration-400 ${isOpen ? '-rotate-45 -translate-y-2': ''} `}></span>
      </button>
    </div>

    {isOpen && (
      <div className="bg-black/90 rounded-b-md absolute right-0 p-10 top-18 w-1/4">

      <ul className={`flex flex-col gap-6 items-center transition duration-400 ${isOpen ? 'translate-x-0': 'translate-x-10'}`}>
      <li><NavLink to='/' className={({isActive}) => isActive ? 'underline' : 'none'}>Home</NavLink></li>
